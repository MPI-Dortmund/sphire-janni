...
def predict_np(model, image, patch_size=(1024, 1024), padding=15,batch_size=4):
    img_patches = utils.image_to_patches(image, patch_size=patch_size, padding=padding)
    print(img_patches.shape)
    denoised_patches = model.predict(x=img_patches[:, :, :, np.newaxis], batch_size=batch_size)

    denoised_micrograph = utils.patches_to_image(denoised_patches, image_shape=image.shape,padding=15)
